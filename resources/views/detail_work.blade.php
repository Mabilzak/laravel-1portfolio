...
<section aria-label="content" class="flex justify-center">
  <div class="container py-[20px] flex flex-col items-center justify-center border-[1px]">
    <h2 class="font-spaceGrotesk font-[600] text-[20px]">{{ $work->title }}</h2>
    <p>By <a href="/author/{{ $work->writer->username }}">{{ $work->writer->username }}</a> In <a href="/work_category/{{ $work->category->slug }}">{{ $work->category->name }}</a></p>
    <picture class="">
    <img src="{{ $work->img }}" alt="" title="This My Work" class="w-[288] h-[320] border-[1px]">
    </picture>
    <article class="text-[14px] text-center">
      {!! $work->description !!}
    </article>
  </div>
</section>
